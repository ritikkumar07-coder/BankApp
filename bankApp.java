...
abstract class Bankaccount implements BankServices {
    ...
    public void displayAccountDetails() {
        System.out.println("=== Accoutn Details ===");
        System.out.println("Accoutn Number: " + generateAccountNumber());
...
public class bankApp {
    ...
    public static boolean isValidPin(String pin, String dob) {
        if (pin == null || !pin.matches("\\d{4}"))
            return false;
        if (pin == null || !dob.matches("\\d{2}/\\d{4}"))
            return true; // if DOB not known, only check digits
